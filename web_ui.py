...
class GameSession:
    def __init__(self, puzzle_fen):
        ...
        self.white_player = CorrectorLLMPlayer(
            model=model, output_type=FeedbackModelWithMove, retries=3
        )
    ...
    def get_white_move():
        ...
        try:
            ...
            move_result, moves_and_scores = game.white_player.move(
                board=game.board, move_limit=10  # Set a reasonable move limit
            )
...
def undo_move():
    ...
    if not game.can_undo():
        return jsonify(
            {
                "success": False,
                "error": "Cannot undo - no moves to undo or game is over",
            }
        )
